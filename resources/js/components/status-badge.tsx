...
import { Check, Clock, AlertTriangle, Trash2, RefreshCw, User } from 'lucide-react';

export type StatusVariant = 'success' | 'info' | 'warning' | 'danger' | 'neutral' | 'indigo';
...
export function StatusBadge({ value, variant, label }: StatusBadgeProps) {
    ...
    if (!variant) {
        if (['available', 'active', 'approved', 'finalized', 'good', 'new', 'success'].includes(normalized)) {
            resolvedVariant = 'success';
            Icon = Check;
        } else if (['assigned', 'issued', 'open', 'info'].includes(normalized)) {
            resolvedVariant = 'info';
            Icon = User;
        } else if (['transferred', 'ptr'].includes(normalized)) {
            resolvedVariant = 'indigo';
            Icon = RefreshCw;
        } else if (['pending', 'pending_review', 'pending_approval', 'review', 'warning'].includes(normalized)) {
            resolvedVariant = 'warning';
            Icon = Clock;
        } else if (['disposed', 'rejected', 'danger', 'unserviceable', 'needs_repair'].includes(normalized)) {
    ...
    const variantStyles = {
        ...
        neutral: 'bg-neutral-100 text-neutral-600 border-neutral-200 dark:bg-neutral-900/50 dark:text-neutral-400 dark:border-neutral-800/50',
    };

    return (
        <Badge variant="outline" className={`inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-medium capitalize ${variantStyles[resolvedVariant]}`}>
